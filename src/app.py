...
votes_db = defaultdict(
    lambda: {"upvotes": 0, "downvotes": 0}
)  # Tracks votes for each article


def research_papers():
    """
    Define query to research papers
    """
    query = "(ISO AND 8800) OR (AI AND DevOps) OR (AI AND Safety) OR \
            (Deep AND LEARNING AND Safety) OR (AI AND (Standart OR Norm)) "
...
def index():
    """
    Main page; by default, show all papers
    """
    ...
    articles_with_votes = [
        {
            **articles_db[article.id],
            ...
            "score": votes_db[article.id]["upvotes"]
            - votes_db[article.id]["downvotes"],
        }
        for article in papers
    ]
...
def filter_papers():
    ...
    filtered_papers = [
        paper
        for paper in papers
        if paper.date.astimezone(None) >= cutoff_date.astimezone(None)
    ]
    ...
    return render_template(
        "index.html", papers=articles_with_votes, filter_days=filter_days
    )


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=1405)
